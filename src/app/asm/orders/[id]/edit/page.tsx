...
import { Trash2, Plus, Loader2, ShoppingCart, User, CreditCard, ArrowLeft } from 'lucide-react';
import { SearchableSelect } from '@/components/ui/searchable-select';

// Types for Catalog Data (Same as Create Page)
...
export default function EditOrderPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [dealersRes, oemsRes, orderRes] = await Promise.all([
                    fetch('/api/dealers'),
                ...
                if (orderRes.ok) {
                    ...
                    reset({
                        ...
                        dealerMobile: order.dealer.mobile || '',
                        paymentType: order.paymentType || 'ADVANCE',
                        creditDays: order.creditDays || undefined,
                        remarks: order.remarks || '',
                        items: order.items.map((item: any) => ({
                            designId: item.designId,
                            productCode: item.design.productCode, // Need to fetch this or store it
                    ...
                    setSubmitError('Failed to load order details');
                }
            } catch (err) {
    ...
    async function onSubmit(data: CreateOrderRequest) {
        ...
        try {
            ...
            router.push(`/asm/orders/${params.id}`);
        } catch (err: any) {
            ...
            <div className="max-w-6xl mx-auto">
                <header className="mb-8 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 mt-1">Update order details.</p>
                    </div>
                </header>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <section className="bg-white rounded-2xl shadow-sm border border-slate-200">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <button
                                type="button"
                                onClick={() => append({ quantity: 1, unitType: 'PCS', designId: '', productCode: '', colorId: null, seatType: null })}
...
// Sub-component for Item Row (Same as Create Page)
function OrderItemRow({ index, control, register, remove, oems, setValue, errors, watch }: any) {
    ...
    // Pre-fill logic for dropdowns when editing would be complex here without knowing the hierarchy.
    // For MVP, the user might need to re-select if they want to change the design.
    // However, the designId is set, so the SearchableSelect for design should show the ID/Label if we had the full list.
    // Since we load lists progressively, pre-filling the OEM/Vehicle/Type dropdowns is tricky without extra data from the API.
    // The current implementation will show the selected Design ID if it's not in the loaded list, which is suboptimal.
    // But for now, let's stick to the basic implementation.
    ...
    return (
        <div className="p-5 border border-slate-200 rounded-xl bg-slate-50/30 relative grid grid-cols-1 md:grid-cols-12 gap-5 items-start hover:border-blue-200 transition-colors">
                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <SearchableSelect
                            ...
                            placeholder={field.value ? "Design Selected" : "Select Design"} // Show placeholder if value exists but list not loaded
                            disabled={!selectedType && !field.value} // Allow if value exists (for display)
                        />
                    )}
                />
            ...
            <div className="md:col-span-1">
                ...
                // disabled={!currentDesign || currentDesign.seatOption !== 'BOTH'} // Enable for now to allow editing existing values
                >
                    <option value="">-</option>
            ...
            <div className="md:col-span-1">
                ...
                {/* Simplified Color Select for Edit Mode - ideally should load colors for the design */}
                <div className="h-[38px] w-full bg-slate-100 border border-slate-200 rounded-lg flex items-center justify-center text-slate-400 text-xs">
                    {/* For MVP Edit, we might need to re-select design to change color properly */}
                    {watch(`items.${index}.colorId`) ? 'Selected' : '-'}
                </div>
            </div>
